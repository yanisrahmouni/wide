...
men <- read_dta(paste0(pat18,"/menage.dta")) %>% 
...
individu_data <- read_dta(paste0(pat18,"/individu.dta")) %>%
  select(ident, identind17, noi, lienpref, lienprrp, enfrp, sexe) %>%
  ...
  select(ident, identind17, noi, liensimpl, sexe, lienpref) %>%
  rename(nop = noi) %>% 
  arrange(ident,nop)

# Charger la table produits et sélectionner uniquement les actifs financiers 
produit_data <- read_dta(paste0(pat18,"/produit.dta")) %>%
  filter(nature=="1") %>% 
...
rp <- read_dta(paste0(pat18,"/produit.dta"))%>%
...
hors_rp <- read_dta(paste0(pat18,"/produit.dta")) %>%
  filter(nature == "2" & logna !="01") %>%
  select(ident, parcj, parpr, montcla)
...
ent <- read_dta(paste0(pat18,"/entreprise.dta"))
...
ent <- ent %>% 
  ...
  mutate(valeur_corr=case_when(
    possess_ent=="ME" & chef3!="" & chef4=="" ~ mtentrep2*2/3,
    possess_ent=="ME" & chef3!="" & chef4!="" ~ mtentrep2/2,
    TRUE ~ mtentrep2)) %>%
...
produits <- read_dta(paste0(pat18,"/produit.dta")) %>%
...
merged_data_immo <- merged_data_immo %>% 
  ...
  select(-c(Wimmo_horsrp_cj_1:Wimmo_horsrp_cj_21),
         -c(Wimmo_horsrp_pr_1:Wimmo_horsrp_pr_21)) %>%
...
saveRDS(wealth_data,"wealth_2018.Rda")
