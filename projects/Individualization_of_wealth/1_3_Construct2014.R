...
men <- read_dta(paste0(pat14,"/menage.dta")) %>% 
  select(ident,pond,sexepr,sexecj,couplepr) %>%
...
individu_data <- read_dta(paste0(pat14,"/individu.dta")) %>%
  select(ident, identind14, noi, lienpref, lienprrp, enfrp, sexe) %>%
  ...
  select(ident, identind14, noi, liensimpl, sexe, lienpref) %>%
  rename(nop = noi) %>% 
  arrange(ident,nop)

# Charger la table produits et sélectionner uniquement les actifs financiers 
produit_data <- read_dta(paste0(pat14,"/produit.dta")) %>%
  filter(nature=="1") %>% 
...
produitlien <- produitlien %>%
  ...
  mutate(finna_r=case_when(
    ...
    finna %in% c("08","09") ~ 3,
    finna %in% c("10","11","12","13","14","15") ~ 4,
...
rp <- read_dta(paste0(pat14,"/produit.dta"))%>%
...
hors_rp <- read_dta(paste0(pat14,"/produit.dta")) %>%
  filter(nature == "2" & logna !="01") %>%
  select(ident, parcj, parpr, montcla)
...
ent <- read_dta(paste0(pat14,"/entreprise.dta"))
...
ent <- ent %>% 
  ...
  mutate(valeur_corr=case_when(
    possess_ent=="ME" & !(chef1 %in% c("01","02")) & chef2 %in% c("01","02") & chef3=="" ~ mtentrep2/2,
    possess_ent=="ME" & chef1 %in% c("01","02") & chef2!="" & !(chef2 %in% c("01","02")) & chef3=="" ~ mtentrep2/2,
    possess_ent=="ME" & chef3!="" ~ mtentrep2*2/3,
    TRUE ~ mtentrep2)) %>%
  select(ident, identent, possess_ent, mtentrep2,valeur_corr)
#Si l'entreprise est détenue avec un fils/une fille (et pas la conjoint.e) on attribue la moitié de la valeur au chef1
# Si l'entreprise est détenue par trois personnes on attribue 2/3 de la valeur au chef1+chef2
...
produits <- read_dta(paste0(pat14,"/produit.dta")) %>%
  filter(nature=="3" | nature=="4") %>%
  select(ident,identpos,nop,profna,montcla,parmen) %>%
  filter(nop=="ME") %>%
...
merged_data_immo <- merged_data_immo %>% 
  ...
  select(-c(Wimmo_horsrp_cj_1:Wimmo_horsrp_cj_29),
         -c(Wimmo_horsrp_pr_1:Wimmo_horsrp_pr_29)) %>%
...
wealth_data <- merged_data_immo %>% select(ident,pond,sexepr,sexecj,couple,starts_with("W") & (ends_with("_h") | ends_with("f")))
...
saveRDS(wealth_data,"wealth_2014.Rda")
