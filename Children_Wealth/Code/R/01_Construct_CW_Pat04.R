...
revfiscal_data <- haven::read_dta(paste0(pat04,"//revenus2004.dta"))            # Load the dataset
revfiscal_data <- subset(revfiscal_data, !is.na(identind))                      # Drop observations where identind is missing
revfiscal_data <- unique(revfiscal_data)                                        # Remove duplicate observations
revfiscal_data <- revfiscal_data[, c("identmen", "identind", "noi", 
                                     "zsali", "zchoi", "ztsai",
                                     "zrsti", "zalri", "zreti", "zrtoi", 
                                     "zperi", "zragi", "zrici", "zrnci")]                                             # Select specific variables
haven::write_dta(revfiscal_data, paste0(temp, "//revfiscal.dta"))               # Save the dataset


individu_data <- haven::read_dta(paste0(pat04, "//INDIVIDU.dta"))               # Load the dataset
individu_data <- individu_data %>% inner_join(revfiscal_data, 
                                               by = c("identind", "noi"))       # Perform merge
z_vars <- grep("^z", names(individu_data), value = TRUE)                        # Recode variables starting with "z" to 0 if they have missing values
individu_data[z_vars][is.na(individu_data[z_vars])] <- 0
haven::write_dta(individu_data, paste0(temp, "//INDIVIDU2004.dta"))             # Save the dataset
...
individu_data$PR           <- individu_data$lien %in% c("1", "2")
individu_data$enfant_PR    <- individu_data$lien == "3" & individu_data$enf != "4"
individu_data$autre_enf_PR <- !individu_data$PR & !individu_data$enfant_PR & individu_data$age < 25
individu_data$autre_PR     <- !individu_data$PR & !individu_data$enfant_PR & individu_data$age >= 25

# RECODE ALL T/F as 1/0
compo_men <- individu_data[ , c("PR","enfant_PR",
                                "autre_enf_PR", "autre_PR",
                                "ident")] %>% 
             dplyr::group_by(ident) %>%
             dplyr::summarise_all(list(sum))                                    # Collapse data by ident
haven::write_dta(compo_men, paste0(temp, "//compo_men.dta"))                    # Save the dataset

##### 1/ ENFANTS 

## 1.0/ identification des enfants (mineurs)

individu_data <- subset(individu_data, age <= 17)                               # Keep observations where age is less than or equal to 17

individu_data$PR        <- individu_data$lien %in% c("1", "2")
individu_data$enfant_PR <- individu_data$lien == "3" & individu_data$enf != "4"

individu_data <- subset(individu_data, enfant_PR != 0)

individu_data$enfant_couple  <- individu_data$lien == "3" & individu_data$enf == "1"
individu_data$enfant_PR_seul <- individu_data$lien == "3" & individu_data$enf == "2"
individu_data$enfant_CJ_seul <- individu_data$lien == "3" & individu_data$enf == "3"

# Select specific variables
individu_data <- individu_data[, c("ident", "identind", "age", "sexe", 
                                   "enfant_PR", "enfant_couple", "enfant_PR_seul",
                                   "enfant_CJ_seul", "occupa", "jepprof", "jemprof",
                                   "cyfran", "posit", "viecou", "matri", "etudi",
                                   "gparpat", "gparmat", "herdon_i", "herdon1_i", 
                                   "herdon2_i", "na1", "tio", "paysnai", "anais",
                                   "pere", "mere", "statut", "cs", "dieg", "diep", 
                                   "dies", "diplo", "pond",
                                   colnames(individu_data)[grep("^z", names(individu_data))], 
                                   "gestsep_i", "duree")]

haven::write_dta(individu_data, paste0(temp, "//children.dta"))                 # Save the dataset
