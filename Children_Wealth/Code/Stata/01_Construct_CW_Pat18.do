...
*------------------------------------------------------------------------------*
*                                                                              *
* 01_Construct_CW_Pat18: prepares data from the 2018 survey.                   *
*                                                                              *
*------------------------------------------------------------------------------*
* Inputs: French Wealth Survey datasets (2018)                                 *
* Outputs: assets_2018.dta                                                     *
*------------------------------------------------------------------------------*
* Creation date: 2020                                                          *
* Update: 03/2024                                                              *
*------------------------------------------------------------------------------*
...
keep ident identind17 identindl indiv_* enfant_couple enfant_PR_seul enfant_CJ_seul ///
...
gestsep_i duree 
ren indiv_panel indiv_panel_2017
ren indiv_reinterro indiv_reinterro_2017
so identind17
save $temp\children, replace

** 1.1/ assets financiers des enfants

use $pat18\PRODUIT, clear
ren *, lower
ren identpos identind17
so identind17
merge m:1 identind17 using $temp\children
keep if _m == 3 | _m == 2
...
collapse (sum) Wfin_cc Wfin_epargne Wfin_nonep has_*, by(identind17)
recode has_* (2/max = 1)
tempfile fin_enf
so identind17
save `fin_enf'
...
drop _merge
so identind17
merge 1:1 identind17 using `fin_enf'
drop _m
...
keep ident identind17 identindl indiv_* age sexe PR lienpref noi actif actoccup anarriv classif ///
...
ren identpos identind17
so identind17
merge m:1 identind17 using $temp\pr_cj
keep if _m == 3 | _m == 2
...
drop if _m == 1
* on vire les couples homo (2 HH et 11 FF)
...
foreach v in noi age Wimmo_brut Wimmo_net Wfin_epargne Wfin_nonep actif ///
			 ...
			 dipdet pond_long_ind zchomage_i zretraites_strictes_i ///
...
so ident 
merge 1:m ident using $temp/assets_2017
drop if _m == 1
drop _m
...
keep ident identind17 age anais sexe enfant_* mnais
so ident anais 
...
keep ident identind17 multiple rank_* nb_girls_* nb_boys_* nb_enf_m nb_enf_17m ///
	 nb_enf_18p nb_enf_all rank_mult_birth1 mult_birth_hh1 rank_mult_birth2 ///
	 mult_birth_hh2 sexe_enf*
so ident identind17

merge 1:1 ident identind17 using $temp/assets_2017
drop if _m == 1
drop _m
