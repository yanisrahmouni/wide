...
*------------------------------------------------------------------------------*
*                                                                              *
* 01_Construct_CW_Pat14: prepares data from the 2014 survey.                   *
*                                                                              *
*------------------------------------------------------------------------------*
* Inputs: French Wealth Survey datasets (2014)                                 *
* Outputs: assets_2014.dta                                                     *
*------------------------------------------------------------------------------*
* Creation date: 2020                                                          *
* Update: 03/2024                                                              *
*------------------------------------------------------------------------------*
...
keep ident identind14 identindl indiv_panel enfant_couple enfant_PR_seul enfant_CJ_seul ///
	 age sexe enfant_PR enfant_couple enfant_PR_seul enfant_CJ_seul actif ///
	 actoccup anarriv classif classifante couple etamatri educstat forter ///
	  hertabd_i hertabh_i rechemploi pacs natio7 autnatio7 nais7 ///
	 anais per1e mer1e statut cs42 cs24 cs_ante  dipdet pond jemprof jepprof ///
	 z* gparpat gparmat hertabd_i hertabh_i  ///
gestsep_i duree 
ren indiv_panel indiv_panel_2014
tempfile children
so identind14 
...
use $pat14\PRODUIT, clear
ren identpos identind14
so identind14
merge m:1 identind14 using $temp\children
keep if _m == 3 | _m == 2
...
collapse (sum) Wfin_cc Wfin_epargne Wfin_nonep has_*, by(identind14)
recode has* (2/max = 1)
...
* => seulement 99 cas => ça ne devrait pas affecter les résultats (à vérifier ??)
* Plus not using real estate wealth for fertilty & children wealth study
...
drop _merge
so identind14
merge 1:1 identind14 using `fin_enf'
drop _m
...
gen enfant_PR = (lienpref == "02" | lienpref == "31")
drop if PR == 0
keep ident identind14 identindl indiv_panel age sexe PR lienpref noi actif actoccup anarriv classif ///
	classifante couple etamat educstat forter  hertabd_i hertabh_i ///
...
reshape wide noi age sexe actif actoccup anarriv classif classifante couple ///
		etamat educstat forter  hertabd_i hertabh_i rechemploi pacs ///
		natio7 autnatio7 nais7 anais per1e mer1e statut cs42 cs24 cs_ante  ///
		dipdet pond z* gparpat gparmat jefrso jeargt jegrave jepprof jemact ///
		jemprof jepro_* ///
...
foreach v in noi age sexe actif actoccup anarriv classif classifante couple ///
			 ...
			 cs_ante  dipdet pond    z* gparpat gparmat jefrso jeargt jegrave ///
...
ren identpos identind14
so identind14
merge m:1 identind14 using $temp\pr_cj
keep if _m == 3 | _m == 2
...
replace immo_valeur_nette = immo_valeur_brute if acqdet == "2"
gen exist_dette_immo = (acqdet == "1")
keep ident identlog immo_valeur_brute immo_valeur_nette parpr parcj paraut ///
	 parhm exist_dette_immo
so ident
...
use $pat14\ENTREPRISE, clear
keep ident identent mtentrep totmin_decl totmax_decl pca kduent kduent_decl ///
	 detmen kdumen ca nat_ca jur r_proent r_terent
...
keep identprod ident identpos dette_pro_men dette_pro_ent kdu kdu_corr ///
...
/* KDU_CORR  A LA PLACE DE KDU ???*/
*list kdu kdu_corr if kdu != kdu_corr
...
drop if _m == 1
* on vire les couples homo (4 HH et 13 FF)
...
foreach v in noi age Wimmo_brut Wimmo_net Wfin_epargne Wfin_nonep actif ///
			 ...
			 dipdet pond zchomage_i zretraites_strictes_i zinvalidite_i ///
			 zrag_i zric_i zrnc_i zretraites_i zrentes_i zpenalir_i ///
			 zsalaires_i gparpat gparmat jefrso jeargt jegrave jepprof jemact ///
			 jemprof jepro_rp jepro_immo jepro_ter jepro_vmob jepro_avi ///
			 jepro_trav jepro_agri ///
gestsep_i duree {
...
* Patrimoine financier (n'inclut pas les compte courants)
...
save $temp/assets_2014, replace


*** variables socio-démographiques

use $pat14/menage, clear 
forv k = 1/12{
...
so ident 
merge 1:m ident using $temp/assets_2014
drop if _m == 1
drop _m
gen nbenf_hm_hseul = .
gen nbenf_hm_fseul = .
...
keep ident identind14 age anais sexe enfant_* mnais
...
so ident anais mnais
drop nenfants
order ident nb_enf_m nb_enf_hm nb_enf_all  nb_m nb_hm rank_all
...
bys ident multiple anais mnais : egen rank_PR = min(rank_PR_temp)
...
keep ident identind14 multiple rank_* nb_girls_* nb_boys_* nb_enf_m ///
	 nb_enf_17m nb_enf_18p nb_enf_all rank_mult_birth1 mult_birth_hh1 ///
	 rank_mult_birth2 mult_birth_hh2 sexe_enf*

so ident identind14
merge 1:1 ident identind14 using $temp/assets_2014
drop if _m == 1
drop _m
...
* Variable milieu social 
* JEFRSOP/JEFRSOC = nb frère et soeur
* JEARGTP/JEARGTC = pb d'argent
* JEGRAVEP/JEGRAVEC = evenements grave 
* JEPPROFP/JEPPROFC = profession du père 
* JEMACTP/JEMACTC = activité prof de la mère 
* JEMPROFP/JEMPROFC = profession de la mère 
* JEPROP1 - JEPROP6/JEPROC1 - JEPROC6 = propriétaire de biens 1 à 6
